...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment, IAuxDetect, IDetect,Decoupled_Detect)):
            s = 256  # 2x min stride
        ...
        elif isinstance(m, IAuxDetect, Decoupled_Detect):
            s = 256  # 2x min stride
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, feature_extraction_block,
                shortcut_block,att_block, ResneXtBottleneckCSP,DenseBlock,PRAM, CBAM,C2f,SpatialAttention,ChannelAttention,
                 C3CA, C3_Res2Block,MobileNet_Block,conv_bn_hswish,CoordAtt,SimAM, ShuffleAttention}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, ResneXtBottleneckCSP,DenseBlock, PRAM, CBAM,C2f, C3CA,
                     SE,SpatialAttention,ChannelAttention,CoordAtt,SimAM, ShuffleAttention}:
        ...
        # TODO: channel, gw, gd
        elif m in {Detect, Segment, Decoupled_Detect, IDetect, IAuxDetect}:
